...
const STRINGS: Record<Lang, Record<string, string>> = {
  en: {
    ...
    "history.close": "Close",
    "dataview.loading": "Running query…",
    "dataview.empty": "No results.",
    "dataview.editQuery": "Edit query",
    "dataview.error.desktop_required": "Dataview requires the desktop app.",
    "dataview.error.sort_field_required": "SORT needs a field.",
    "dataview.error.table_columns_required": "TABLE needs at least one column.",
    "dataview.error.unsupported_query": "Supported Dataview queries are TABLE and LIST.",
    "dataview.error.where_comparison_required": "WHERE needs a comparison such as `rating >= 8`.",
    "dataview.error.unknown": "The Dataview query could not be rendered.",
    ...
    "graph.legendSizeTerms": "written more often",
    ...
    "graph.conceptsOffline": "{n} notes could not be read: their contents are not on this disk. Their terms are missing from the map.",
    "graph.conceptNotes": "in {notes} notes · written {times} times",
    "graph.conceptMore": "and {n} more.",
    "graph.conceptNoNotes": "No notes came back for this term.",
    ...
    "theme.dark": "Dark",
    "settings.colorsHint":
      "Light and dark each have their own colours. Both are editable here, whichever one you are looking at.",
    "settings.colorsActive": "on screen",
    "settings.resetMode": "Reset",
    "settings.color.bg": "Background",
    "settings.color.panel": "Panels",
    "settings.color.ink": "Text",
    "settings.color.muted": "Secondary text",
    "settings.color.accent": "Accent",
    "settings.color.ai": "AI notes",
    "settings.color.highlight": "Text highlight",
    "settings.contrastRatio": "({ratio}:1, needs 4.5:1)",
    "settings.contrast.inkOnBg": "Text is hard to read on the background.",
    "settings.contrast.inkOnPanel": "Text is hard to read on panels.",
    "settings.contrast.mutedOnBg": "Secondary text is hard to read on the background.",
    "settings.contrast.mutedOnPanel": "Secondary text is hard to read on panels.",
    "settings.contrast.inkOnHighlight": "Highlighted text is hard to read.",
    ...
    "settings.importConnecting": "Contacting the site…",
    "settings.importFetching": "{done} posts found…",
    "settings.importAuthorLookup": "Looking up author names — feed page {done} of {total}…",
    "settings.importPreparing": "Preparing {done} posts and reading the vault…",
    "settings.importWriting": "Writing note {done} of {total}…",
    "settings.importNoVault": "Open a vault first — there is nowhere to import to.",
    "settings.importNoUrl": "Enter the address of the blog to import.",
    ...
    "settings.indexBody": "Passages have to be encoded once before a search can use them. This takes a few minutes on a large vault, reports as it goes, and is saved along the way — a run that is cut short picks up where it stopped. Notes you add later are covered by running it again.",
    "settings.indexRun": "Index vault",
    "settings.indexBusy": "Indexing…",
    "settings.indexProgress": "{done} of {total} passages",
    "settings.indexEta": "about {minutes} min left",
    "settings.indexDone": "{count} passages indexed.",
    "settings.indexEmpty": "No passages found in {vault}. That folder holds no readable notes \u2014 check that it is the vault you meant, and that its files are on this machine rather than only in the cloud.",
    "settings.indexSaw": "It listed {notes} notes, {unreadable} of which gave back no text ({offline} of those are cloud placeholders).",
    "settings.indexAllOffline": "All {notes} notes in {vault} are cloud placeholders \u2014 their contents are not on this Mac. iCloud has evicted them to free up space. Download the folder in Finder (right-click \u2192 Download Now) or run `brctl download` on it, then index again. To stop it happening again, turn off \u201cOptimise Mac Storage\u201d for iCloud Drive, or keep the vault outside iCloud.",
    "settings.indexSkipped": "{offline} of {notes} notes were skipped: they are cloud placeholders whose contents are not on this Mac. Search will answer from the rest until they are downloaded.",
    "settings.semanticBody": "Retrieval finds passages by the words they contain. With a language model on disk it also finds passages that mean the same thing in other words. The model is downloaded once, stays on this computer and is used offline; without it everything works as before, just by words alone.",
    "settings.semanticDownload": "Download model ({size} MB)",
    "settings.semanticDownloadUnknown": "Download model (size unknown)",
    "settings.rerankTitle": "Reranking",
    "settings.rerankBody": "A second, smaller model reads the question and the passage together and reorders the best hits. It catches the detail the search model reads past \u2014 \u201clockige schwarze Haare\u201d next to \u201ccurly black long hair\u201d. Costs a second or three per query, and no re-indexing.",
    "settings.rerankReady": "Reranking is on ({model}).",
    "settings.rerankDownload": "Download reranker ({size} MB)",
    "settings.rerankDownloadUnknown": "Download reranker (size unknown)",
    "settings.semanticBusy": "Downloading…",
    "settings.semanticProgress": "{file}: {done} of {total} MB",
    "settings.semanticReady": "Model {model} is installed. Retrieval ranks meaning alongside words.",
  ...
  de: {
    ...
    "dataview.loading": "Query läuft…",
    "dataview.empty": "Keine Treffer.",
    "dataview.editQuery": "Query bearbeiten",
    "dataview.error.desktop_required": "Dataview braucht die Desktop-App.",
    "dataview.error.sort_field_required": "SORT braucht ein Feld.",
    "dataview.error.table_columns_required": "TABLE braucht mindestens eine Spalte.",
    "dataview.error.unsupported_query": "Unterstützt sind TABLE und LIST.",
    "dataview.error.where_comparison_required": "WHERE braucht einen Vergleich wie `rating >= 8`.",
    "dataview.error.unknown": "Die Dataview-Query konnte nicht angezeigt werden.",
    ...
    "graph.legendSizeTerms": "häufiger geschrieben",
    ...
    "graph.conceptsOffline": "{n} Notizen konnten nicht gelesen werden: ihr Inhalt liegt nicht auf dieser Festplatte. Ihre Begriffe fehlen in der Karte.",
    "graph.conceptNotes": "in {notes} Notizen · {times}× geschrieben",
    "graph.conceptMore": "und {n} weitere.",
    "graph.conceptNoNotes": "Zu diesem Begriff kamen keine Notizen zurück.",
    ...
    "theme.dark": "Dunkel",
    "settings.colorsHint":
      "Hell und Dunkel haben eigene Farben. Beide lassen sich hier bearbeiten, egal welche gerade zu sehen ist.",
    "settings.colorsActive": "zu sehen",
    "settings.resetMode": "Zurücksetzen",
    "settings.color.bg": "Hintergrund",
    "settings.color.panel": "Flächen",
    "settings.color.ink": "Text",
    "settings.color.muted": "Nebentext",
    "settings.color.accent": "Akzent",
    "settings.color.ai": "KI-Notizen",
    "settings.color.highlight": "Textmarkierung",
    "settings.contrastRatio": "({ratio}:1, nötig sind 4,5:1)",
    "settings.contrast.inkOnBg": "Text ist auf dem Hintergrund schwer zu lesen.",
    "settings.contrast.inkOnPanel": "Text ist auf Flächen schwer zu lesen.",
    "settings.contrast.mutedOnBg": "Nebentext ist auf dem Hintergrund schwer zu lesen.",
    "settings.contrast.mutedOnPanel": "Nebentext ist auf Flächen schwer zu lesen.",
    "settings.contrast.inkOnHighlight": "Markierter Text ist schwer zu lesen.",
    ...
    "settings.importConnecting": "Seite wird kontaktiert …",
    "settings.importFetching": "{done} Beiträge gefunden …",
    "settings.importAuthorLookup": "Autorennamen werden gesucht — Feed-Seite {done} von {total} …",
    "settings.importPreparing": "{done} Beiträge werden vorbereitet, Vault wird gelesen …",
    "settings.importWriting": "Notiz {done} von {total} wird geschrieben …",
    "settings.importNoVault": "Öffne zuerst einen Vault — es gibt kein Ziel für den Import.",
    "settings.importNoUrl": "Gib die Adresse des Blogs ein, der importiert werden soll.",
    "settings.importing": "Importiere… das kann eine Minute dauern",
    "settings.importDone": "{count} Notizen in „{folder}“ importiert.",
    "settings.semanticTitle": "Semantische Suche",
    "settings.indexBody": "Passagen müssen einmal eingebettet werden, bevor die Suche sie verwenden kann. Das dauert bei einem großen Vault einige Minuten, meldet den Fortschritt und wird laufend gespeichert. Ein abgebrochener Durchlauf setzt beim nächsten Mal dort fort, wo er aufgehört hat. Später hinzugekommene Notizen holt ein erneuter Durchlauf nach.",
    "settings.indexRun": "Vault indizieren",
    "settings.indexBusy": "Wird indiziert …",
    "settings.indexProgress": "{done} von {total} Passagen",
    "settings.indexEta": "noch etwa {minutes} min",
    "settings.indexDone": "{count} Passagen indiziert.",
    "settings.indexEmpty": "Keine Passagen in {vault} gefunden. Dieser Ordner enth\u00e4lt keine lesbaren Notizen \u2014 pr\u00fcfe, ob es der gemeinte Vault ist und ob seine Dateien auf diesem Rechner liegen und nicht nur in der Cloud.",
    "settings.indexSaw": "Aufgelistet wurden {notes} Notizen, davon lieferten {unreadable} keinen Text ({offline} davon sind Cloud-Platzhalter).",
    "settings.indexAllOffline": "Alle {notes} Notizen in {vault} sind Cloud-Platzhalter \u2014 ihre Inhalte liegen nicht auf diesem Mac. iCloud hat sie ausgelagert, um Platz zu schaffen. Lade den Ordner im Finder herunter (Rechtsklick \u2192 \u201eJetzt laden\u201c) oder mit `brctl download`, dann indiziere erneut. Damit es nicht wiederkommt: \u201eMac-Speicher optimieren\u201c f\u00fcr iCloud Drive ausschalten oder den Vault aus iCloud herausnehmen.",
    "settings.indexSkipped": "{offline} von {notes} Notizen wurden \u00fcbersprungen: Cloud-Platzhalter, deren Inhalt nicht auf diesem Mac liegt. Bis sie geladen sind, antwortet die Suche nur aus dem Rest.",
    "settings.semanticBody": "Die Suche findet Passagen über die Wörter, die darin vorkommen. Mit einem Sprachmodell auf der Platte findet sie zusätzlich Passagen, die dasselbe mit anderen Worten sagen. Das Modell wird einmal geladen, bleibt auf diesem Rechner und arbeitet offline. Ohne es funktioniert alles wie bisher, eben nur über Wörter.",
    "settings.semanticDownload": "Modell laden ({size} MB)",
    "settings.semanticDownloadUnknown": "Modell laden (Gr\u00f6\u00dfe unbekannt)",
    "settings.rerankTitle": "Nachsortierung",
    "settings.rerankBody": "Ein zweites, kleineres Modell liest Frage und Passage gemeinsam und sortiert die besten Treffer neu. Es findet Details, \u00fcber die das Suchmodell hinwegliest \u2014 \u201elockige schwarze Haare\u201c neben \u201ecurly black long hair\u201c. Kostet ein bis drei Sekunden pro Abfrage und keine Neuindizierung.",
    "settings.rerankReady": "Nachsortierung aktiv ({model}).",
    "settings.rerankDownload": "Nachsortierung laden ({size} MB)",
    "settings.rerankDownloadUnknown": "Nachsortierung laden (Gr\u00f6\u00dfe unbekannt)",
    "settings.semanticBusy": "Wird geladen …",
    "settings.semanticProgress": "{file}: {done} von {total} MB",
    "settings.semanticReady": "Modell {model} ist installiert. Die Suche bewertet Bedeutung zusätzlich zu Wörtern.",
